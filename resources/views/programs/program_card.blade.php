<div class="bd-program @if (!isset($homePage)) clr{{ $clr['1'] }} @endif mb-40 wow fadeInUp h-100"
    data-wow-duration="1s" data-wow-delay=".2s">
    <div class="bd-program-thumb-wrapper">
        <a href="{{ route('programs.detail', [$program->category->slug, $program->slug]) }}">
            <!-- <div class="bd-program-thumb">
                <img src="{{ asset($program->images->first()->path ?? 'assets/img/program/home-program-1.jpg') }}"
                    alt="Image not found">
            </div> -->
            <div style="padding: 10px;">
                <div  id="cutom-hover-effect"
                style="
                background-image: url('{{ asset($program->images->first()->path ?? 'assets/img/program/home-program-1.jpg') }}');
                ">
                </div>
            </div>
        </a>
    ...
    <div class="bd-program-content mb-40">
        <h4 class="bd-program-title">
            <span style="font-size: 12px;" class="btn btn-primary rounded btn-sm">{{ $program->category->title }}</span> <br>
            <a class="hover-clr{{ $clr['1'] }}"
                href="{{ route('programs.detail', [$program->category->slug, $program->slug]) }}">{{ $program->title }}</a>
        </h4>
        <p>{{ \Illuminate\Support\Str::limit($program->short_desc, 150, $end='...') }}</p>
    </div>
    @forelse ($program->groups as $group)
        <div class="bd-program-info-wrapper theme-bg{{ $clr['0'] }}">
            <div class="bd-program-info">
                <h5 class="bd-program-info-title">{{ $group->age_group }} Years <br><span>age</span></h5>
            </div>

            <div class="bd-program-info">
                <h5 class="bd-program-info-title">{{ $group->price() }}<br><span>price</span></h5>
            </div>
        </div>
        <div class="bd-program-info-wrapper mt-2 theme-bg{{ $clr['0'] }}">
            <h5 class="bd-program-info-title">Date : <span>{{ $group->date() }}</span></h5>
        </div>
        <div class="bd-program-info-wrapper mt-2 bg-dark bg-opacity-75 text-white">
            <a class="text-center" href="{{ route('programs.detail', [$program->category->slug, $program->slug]) }}">View More</a>
        </div>
        @if(!$loop->last)
        <div class="text-center mt-2">
            <b >Other Age group ?</b>
            <small class="text_orange"><a href="{{ route('programs.detail', [$program->category->slug, $program->slug]).'#BookNow' }}">
                Click here to see availabilties.
            </a></small>
        </div>
        @break
        @endif
    ...
    <div class="bd-program-info-wrapper theme-bg{{ $clr['0'] }}">
        <div class="bd-program-info">
            <h5 class="bd-program-info-title">No Group Available For Booking</h5>
        </div>
    </div>
    @endforelse
